    ...
    <div style={{ minHeight: '100vh', background: '#F5F4F0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <div style={{ width: '100%', maxWidth: 380, padding: '0 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 15, color: 'rgba(15,30,60,0.5)', marginTop: 10 }}>Reset your password</div>
        </div>
        <div style={{ background: 'white', borderRadius: 20, border: '1px solid rgba(15,30,60,0.1)', padding: 32 }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '16px 0' }}>
              <div style={{ fontSize: 28, marginBottom: 12 }}>✉️</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#0F1E3C', marginBottom: 6 }}>Check your email</div>
              <div style={{ fontSize: 13, color: 'rgba(15,30,60,0.5)', lineHeight: 1.6 }}>We sent a password reset link to {email}</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button onClick={() => setSent(true)}
                style={{ padding: '12px', borderRadius: 10, background: '#0F1E3C', color: 'white', fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer' }}>
                Send reset link
              </button>
            </div>
          )}
          <div style={{ marginTop: 20, textAlign: 'center', fontSize: 13, color: 'rgba(15,30,60,0.45)' }}>
            <Link href="/login" style={{ color: '#0F1E3C', textDecoration: 'none', fontWeight: 600 }}>Back to sign in</Link>
          </div>
        </div>
      </div>
    </div>
