  ...
  return (
    <div>
      ...
      <div style={{ padding: '28px 36px' }}>
        ...
        <div style={{ ...S.card, marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(26,47,26,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 16 }}>Rate stress test — current rate {inputs.interestRate}%</div>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(26,47,26,0.08)' }}>
                {['Rate rise', 'New rate', 'After-tax /wk', 'Change /wk', 'Annual impact', 'ATO refund', 'Break-even rent'].map(h => (
                  <th key={h} style={{ padding: '8px 12px', textAlign: 'right', fontSize: 10, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.06em', color: 'rgba(26,47,26,0.4)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {results.map(r => {
                ...
                return (
                  <tr key={r.rise} style={{ background: isBase ? 'rgba(201,150,58,0.06)' : 'transparent', borderBottom: '1px solid rgba(26,47,26,0.05)' }}>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: isBase ? 700 : 500, color: C.forest, fontSize: 13 }}>{isBase ? 'Current' : `+${r.rise}%`}</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'monospace', fontSize: 13, color: C.forest }}>{r.rate.toFixed(2)}%</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'monospace', fontSize: 13, fontWeight: 600, color: r.result.weeklyAfterTaxCashFlow >= 0 ? '#16A34A' : '#EF4444' }}>{fmtCurrency(r.result.weeklyAfterTaxCashFlow)}/wk</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'monospace', fontSize: 13, color: weeklyChange <= 0 ? '#EF4444' : '#16A34A' }}>{isBase ? '—' : `${fmtCurrency(weeklyChange)}/wk`}</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'monospace', fontSize: 13, color: weeklyChange <= 0 ? '#EF4444' : '#16A34A' }}>{isBase ? '—' : fmtCurrency(weeklyChange * 52)}</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'monospace', fontSize: 13, color: C.gold }}>{fmtCurrency(r.result.taxRefund)}</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'monospace', fontSize: 13, color: C.forest }}>{fmtCurrency(r.result.breakEvenRent)}/wk</td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <div style={{ ...S.card }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(26,47,26,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 16 }}>Weekly after-tax cash flow by rate</div>
            ...
            {results.map(r => {
              ...
              return (
                <div key={r.rise} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{ fontSize: 10, fontFamily: 'monospace', color: isNeg ? '#EF4444' : '#16A34A', fontWeight: 600 }}>{fmtCurrency(val)}</div>
                  <div style={{ width: '100%', height: `${Math.max(pct * 80, 4)}px`, borderRadius: 4, background: isNeg ? '#EF4444' : '#16A34A', opacity: r.rise === 0 ? 1 : 0.7 }} />
                  <div style={{ fontSize: 10, color: 'rgba(26,47,26,0.5)', fontWeight: r.rise === 0 ? 700 : 400 }}>{r.rise === 0 ? 'Now' : `+${r.rise}%`}</div>
                </div>
              )
            })}
          </div>
