...
export default function HoldSellPage() {
  ...
  const [annualCF, setAnnualCF]           = useState(-9000)
  ...
  const scenarios = useMemo(() =>
    HOLD_YEARS.map(extra => {
      ...
      const totalCFCost = annualCF * extra
    ...
    }), [purchasePrice, purchaseYear, growthRate, taxableIncome, loanBalance, improvements, annualCF, yearsHeld, marginalRate, costBase, currentYear])
  ...
  return (
    <div>
      ...
      <div style={{ padding: '28px 36px' }}>
        <div style={{ ...S.card, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
            {[
              ...
              { label: 'After-tax CF /yr (current)', val: annualCF, set: setAnnualCF, prefix: '$', step: 1000 },
        ...
        <p style={{ fontSize: 11, color: 'rgba(26,47,26,0.35)', textAlign: 'center', marginTop: 16 }}>Estimates only. Does not account for main residence exemption or prior capital losses. Consult a registered tax agent.</p>
      </div>
    </div>
  )
}
