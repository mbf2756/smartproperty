...
const SCENARIOS: Scenario[] = [
  ...
  { label: 'Base', growthRate: 5.5, interestRate: 6.2, color: C.gold },
...
export default function ScenarioPage() {
  ...
  if (checked && !isPaid) {
    return (
      <div>
        <DashboardTopbar title="Scenario Comparison" subtitle="Investor plan required" />
        <LockedPage title="Scenario Comparison" icon="⇄" description="Run Bear, Base and Bull growth cases side by side — plus IO vs P&I comparison. See which combination of loan type, growth rate and hold period maximises your after-tax return." plan="investor" />
      </div>
    )
  }
      ...
      <div style={{ padding: '28px 36px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 20 }}>
          {results.map(r => {
            ...
            return (
              <div key={r.label} style={{ ...S.card, border: `2px solid ${r.color}20` }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 0, background: C.forest, borderRadius: 12, padding: '16px' }}>
                  {[
        ...
        {/* Side-by-side IO vs PI for base case */}
        <div style={{ ...S.card }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
            {[results[1].io, results[1].pi].map((res, i) => (
              <div key={i} style={{ background: C.cream, borderRadius: 12, padding: '16px' }}>
                <div style={{ fontWeight: 700, color: C.forest, marginBottom: 12, fontSize: 14 }}>
                  {i === 0 ? 'Interest Only' : 'Principal & Interest'}
                </div>
                {[
