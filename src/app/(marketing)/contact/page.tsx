...
export default function ContactPage() {
  ...
  const inputStyle = { width: '100%', padding: '10px 14px', border: '1px solid rgba(15,30,60,0.12)', borderRadius: 10, fontSize: 14, color: '#0F1E3C', outline: 'none', background: 'white', boxSizing: 'border-box' as const }
  const labelStyle = { display: 'block', fontSize: 11, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.08em', color: 'rgba(15,30,60,0.5)', marginBottom: 6 }

  return (
    <div style={{ minHeight: '100vh', background: '#F5F4F0', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <nav style={{ background: '#F5F4F0', padding: '20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(15,30,60,0.06)', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          <Link href="/login" style={{ fontSize: 13, color: '#0F1E3C', textDecoration: 'none', padding: '9px 18px', borderRadius: 9, fontWeight: 600, border: '1.5px solid rgba(15,30,60,0.2)', background: 'white' }}>Sign in</Link>
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '64px 40px' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#00A888', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>Contact</div>
          <h1 style={{ fontSize: 34, fontWeight: 800, color: '#0F1E3C', letterSpacing: '-0.03em', marginBottom: 12 }}>Get in touch</h1>
          <p style={{ fontSize: 15, color: 'rgba(15,30,60,0.5)', lineHeight: 1.6 }}>
            Questions about SmartProperty, the Smart Suite, or broker pricing? We&apos;re based in Brisbane, Australia.
          </p>
        </div>

        {success ? (
          <div style={{ background: 'white', borderRadius: 20, border: '1px solid rgba(15,30,60,0.1)', padding: '48px 32px', textAlign: 'center' }}>
            <div style={{ fontSize: 40, marginBottom: 16 }}>✉️</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#0F1E3C', marginBottom: 8 }}>Message sent!</div>
            <div style={{ fontSize: 14, color: 'rgba(15,30,60,0.5)', lineHeight: 1.6 }}>
              We&apos;ll get back to you within 1–2 business days (AEST).
            </div>
            <button onClick={() => setSuccess(false)}
              style={{ marginTop: 20, padding: '10px 20px', borderRadius: 10, background: '#F5F4F0', border: '1px solid rgba(15,30,60,0.12)', fontSize: 13, color: '#0F1E3C', cursor: 'pointer', fontWeight: 600 }}>
          ...
          <div style={{ background: 'white', borderRadius: 20, border: '1px solid rgba(15,30,60,0.1)', padding: 32 }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button type="submit" disabled={loading}
                style={{ padding: '12px', borderRadius: 10, background: '#0F1E3C', color: '#00D4AA', fontWeight: 800, fontSize: 14, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
