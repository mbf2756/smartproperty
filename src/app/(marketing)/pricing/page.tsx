    ...
    <div style={{ minHeight: '100vh', background: '#F5F4F0', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <nav style={{ background: '#F5F4F0', padding: '20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50, borderBottom: '1px solid rgba(15,30,60,0.06)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <Link href="/" style={{ fontSize: 13, color: 'rgba(15,30,60,0.55)', textDecoration: 'none', padding: '8px 14px', fontWeight: 500 }}>Home</Link>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '64px 40px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#00A888', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Simple pricing</div>
          <h1 style={{ fontSize: 40, fontWeight: 800, color: '#0F1E3C', letterSpacing: '-0.04em', lineHeight: 1.1, marginBottom: 16 }}>
            Less than one hour of accountant time.
          </h1>
          <p style={{ fontSize: 16, color: 'rgba(15,30,60,0.55)', maxWidth: 480, margin: '0 auto', lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 56 }}>
          {/* Free */}
          <div style={{ background: 'white', borderRadius: 20, padding: '28px', border: '1px solid rgba(15,30,60,0.1)' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(15,30,60,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>Free</div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 6 }}>
              <span style={{ fontFamily: 'monospace', fontSize: 36, fontWeight: 700, color: '#0F1E3C' }}>$0</span>
              <span style={{ fontSize: 13, color: 'rgba(15,30,60,0.4)' }}>/month</span>
            </div>
            <p style={{ fontSize: 13, color: 'rgba(15,30,60,0.5)', marginBottom: 20, lineHeight: 1.5 }}>No credit card. No time limit.</p>
            <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 24px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {FREE_FEATURES.map(f => (
                <li key={f.label} style={{ display: 'flex', gap: 8, fontSize: 13, color: '#0F1E3C' }}>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, marginTop: 2 }}>
                    <circle cx="7" cy="7" r="6.5" stroke="#00D4AA" strokeWidth="1.2"/>
                    <path d="M4.5 7l2 2 3-3" stroke="#00D4AA" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  {f.label}
                </li>
              ))}
            </ul>
            <Link href="/signup" style={{ display: 'block', textAlign: 'center', padding: '12px', borderRadius: 12, border: '1.5px solid rgba(15,30,60,0.15)', fontSize: 14, fontWeight: 700, color: '#0F1E3C', textDecoration: 'none' }}>
              Get started free
            </Link>
          </div>

          {/* Investor */}
          <div style={{ background: '#0F1E3C', borderRadius: 20, padding: '28px', position: 'relative' }}>
            <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#00D4AA', color: '#0F1E3C', fontSize: 10, fontWeight: 800, padding: '4px 12px', borderRadius: 20, whiteSpace: 'nowrap' }}>MOST POPULAR</div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 4 }}>
              <span style={{ fontFamily: 'monospace', fontSize: 36, fontWeight: 700, color: '#00D4AA' }}>$29</span>
            ...
            <Link href="/signup" style={{ display: 'block', textAlign: 'center', padding: '12px', borderRadius: 12, background: '#00D4AA', fontSize: 14, fontWeight: 800, color: '#0F1E3C', textDecoration: 'none' }}>
              Start free trial →
            </Link>
          </div>
          ...
          {/* Advisor */}
          <div style={{ background: 'white', borderRadius: 20, padding: '28px', border: '1px solid rgba(15,30,60,0.1)' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(15,30,60,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>Advisor</div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 6 }}>
              <span style={{ fontFamily: 'monospace', fontSize: 36, fontWeight: 700, color: '#0F1E3C' }}>$199</span>
              <span style={{ fontSize: 13, color: 'rgba(15,30,60,0.4)' }}>/month</span>
            </div>
            <p style={{ fontSize: 13, color: 'rgba(15,30,60,0.5)', marginBottom: 20, lineHeight: 1.5 }}>For mortgage brokers, buyer's agents & accountants.</p>
            <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 24px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {['Everything in Investor', 'Team seats (up to 5 users)', 'White-label PDF client reports', 'API access', 'SmartETF + SmartSuper bundle', 'Dedicated account manager'].map(f => (
                <li key={f} style={{ display: 'flex', gap: 8, fontSize: 13, color: '#0F1E3C' }}>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, marginTop: 2 }}>
                    <circle cx="7" cy="7" r="6.5" stroke="#00D4AA" strokeWidth="1.2"/>
                    <path d="M4.5 7l2 2 3-3" stroke="#00D4AA" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
            ...
            <Link href="/contact" style={{ display: 'block', textAlign: 'center', padding: '12px', borderRadius: 12, border: '1.5px solid rgba(15,30,60,0.15)', fontSize: 14, fontWeight: 700, color: '#0F1E3C', textDecoration: 'none' }}>
              Contact us
            </Link>
          </div>
        </div>
        ...
        {/* Smart Suite bundle */}
        <div style={{ background: '#0F1E3C', borderRadius: 20, padding: '32px', marginBottom: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 40 }}>
          <div>
            <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#00D4AA', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8 }}>Smart Suite bundle</div>
          ...
          <div style={{ flexShrink: 0, textAlign: 'center' }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {['SmartETF', 'SmartSuper', 'SmartProperty'].map((n, i) => (
                <div key={n} style={{ padding: '8px 14px', borderRadius: 10, background: i === 2 ? 'rgba(0,212,170,0.2)' : 'rgba(255,255,255,0.05)', border: i === 2 ? '1px solid rgba(0,212,170,0.4)' : '1px solid rgba(255,255,255,0.1)', fontSize: 12, fontWeight: 700, color: i === 2 ? '#00D4AA' : 'rgba(255,255,255,0.5)' }}>
                  {n}
                </div>
              ))}
            </div>
            <Link href="/contact" style={{ display: 'inline-block', padding: '10px 24px', background: '#00D4AA', borderRadius: 10, fontSize: 13, fontWeight: 800, color: '#0F1E3C', textDecoration: 'none' }}>
              Get bundle pricing →
            </Link>
          </div>
        </div>
        ...
        {/* FAQ */}
        <div>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: '#0F1E3C', letterSpacing: '-0.03em', textAlign: 'center', marginBottom: 36 }}>Frequently asked questions</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {FAQ.map(f => (
              <div key={f.q} style={{ background: 'white', borderRadius: 14, padding: '20px', border: '1px solid rgba(15,30,60,0.08)' }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#0F1E3C', marginBottom: 8 }}>{f.q}</div>
                <div style={{ fontSize: 13, color: 'rgba(15,30,60,0.6)', lineHeight: 1.6 }}>{f.a}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <footer style={{ background: '#0F1E3C', padding: '40px 40px 24px', marginTop: 64 }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#00D4AA', letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 4 }}>AU · PROPERTY</div>
