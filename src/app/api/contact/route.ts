...
export async function POST(request: Request) {
  ...
  try {
    ...
    // DB logging failure shouldn't block email
  }
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:32px 20px">
        <div style="background:#0F1E3C;padding:20px 24px;border-radius:12px 12px 0 0">
          <div style="color:#00D4AA;font-size:11px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:4px">SmartProperty · Contact Form</div>
        ...
        <div style="background:white;border:1px solid #eee;border-top:none;border-radius:0 0 12px 12px;padding:24px">
          <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
            ...
            <tr><td style="padding:8px 0;border-bottom:1px solid #f0f0f0;color:#8A9BB5;font-size:13px">Email</td>
                <td style="padding:8px 0;border-bottom:1px solid #f0f0f0;color:#0F1E3C;font-size:13px"><a href="mailto:${email.trim()}" style="color:#00A888">${email.trim()}</a></td></tr>
            <tr><td style="padding:8px 0;color:#8A9BB5;font-size:13px">Category</td>
                <td style="padding:8px 0;color:#0F1E3C;font-size:13px">${category || 'General'}</td></tr>
          </table>
          ...
          <div style="margin-top:20px">
            <a href="mailto:${email.trim()}?subject=Re: ${encodeURIComponent(subject?.trim() || 'Your SmartProperty enquiry')}"
        ...
        <div style="margin-top:16px;font-size:11px;color:#8A9BB5;text-align:center">SmartProperty · Brisbane, Australia</div>
      </div>
    `,
  })
  ...
  await resend.emails.send({
    from: fromEmail,
    to: email.trim(),
    subject: `We received your message — SmartProperty`,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:40px auto;padding:20px">
        <div style="background:#0F1E3C;padding:20px 24px;border-radius:12px;margin-bottom:20px;text-align:center">
          <div style="color:#00D4AA;font-size:11px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:4px">AU · PROPERTY</div>
          <div style="color:white;font-size:20px;font-weight:600">SmartProperty</div>
        </div>
        ...
        <p style="color:#4A5F7A;line-height:1.7;margin-bottom:16px">
          ...
          Our team is based in Brisbane, Australia — replies come from AEST business hours.
        </p>
        ...
        <p style="color:#4A5F7A;line-height:1.7;font-size:14px">
          While you wait, you can <a href="${appUrl}/dashboard" style="color:#00A888">log in to your dashboard</a>
          or <a href="${appUrl}/pricing" style="color:#00A888">view our plans</a>.
        </p>
        <hr style="border:none;border-top:1px solid #eee;margin:24px 0">
        <p style="font-size:11px;color:#8A9BB5;line-height:1.6">
          SmartProperty · Brisbane, Australia<br>
